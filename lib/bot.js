...
import { checkIn, checkOut, markLate, addCheckoutNote } from './checkin.js';
...
const WORK_UPDATE_PROMPT = '📝 What did you work on today? Reply here with a quick update.';
...
// force_reply on the work-update prompt so Levin's client puts him straight
// into reply mode instead of needing to long-press the message.
async function replyCheckout(ctx, text) {
  if (text.includes(WORK_UPDATE_PROMPT)) {
    return ctx.reply(text, { reply_markup: { force_reply: true, selective: true } });
  }
...
bot.command('checkout', async (ctx) => {
  ...
  try {
    await replyCheckout(ctx, await doCheckOut(ctx, person, parsed.time));
  } catch (err) {
...
bot.on(message('text'), async (ctx, next) => {
  ...
  if (prompt.endsWith(WORK_UPDATE_PROMPT)) {
    const person = resolvePerson(ctx);
    if (!person) return next();
    if (!text) return ctx.reply("That came through empty — send a quick line about today's work.");
    try {
      await addCheckoutNote(person, text);
      await ctx.reply(`Got it, thanks ${person}! Logged your update.`);
    } catch (err) {
      console.error('checkout update error:', err);
      await ctx.reply('Could not save that. Check server logs.');
    }
    return;
  }
