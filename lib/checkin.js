...
// Captures a reply to the post-checkout "what did you work on today?" prompt
// onto today's check-in row, so it shows up in the note wherever notes do.
export async function addCheckoutNote(person, text) {
  const today = todayIST();
  let row = await findTodayRow(person, today);
  if (!row) row = await createTodayRow(person, today);
  const entry = `Update: ${text}`;
  await notion.pages.update({
    page_id: row.id,
    properties: noteProp(row.note ? `${row.note}; ${entry}` : entry),
  });
  return { status: 'ok' };
}
