...
const CHECKIN_ROSTER = ['Jishnu', 'Levin'];
...
export async function checkIn(person, timeOverride) {
...
export async function missingCheckins() {
  ...
  return CHECKIN_ROSTER.filter((p) => !checkedIn.has(p));
}
...
export async function missingCheckouts() {
  ...
  return {
    ...
    neverCheckedIn: CHECKIN_ROSTER.filter((p) => !checkedInNames.has(p)),
  };
}

// Full snapshot of today's check-in state, for the 19:00 IST status post.
export async function todayCheckinStatus() {
  ...
  return {
    date: todayIST(),
    checkedIn,
    notCheckedIn: CHECKIN_ROSTER.filter((p) => !checkedInNames.has(p)),
