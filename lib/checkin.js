...
export async function missingCheckins() {
  ...
  return CHECKIN_ROSTER.filter((p) => !checkedIn.has(p)).map((p) => (notes[p] ? `${p} (${notes[p]})` : p));
}
...
export async function todayCheckinStatus() {
  ...
  return {
    date: todayIST(),
    checkedIn,
    notCheckedIn: CHECKIN_ROSTER.filter((p) => !checkedInNames.has(p)).map((p) => {
...
// late count (after LATE_AFTER), WFH count. monthStr = "YYYY-MM".
