...
export function formatCheckinStatus({ date, checkedIn, notCheckedIn, notCheckedOut }) {
  const lines = [`🕒 <b>Check-in status</b> — ${formatDate(date)}`, ''];

  if (checkedIn.length > 0) {
    lines.push('✅ <b>Checked in</b>');
    for (const c of checkedIn) {
      lines.push(`• ${c.person} — ${c.checkIn}${c.checkOut ? ` → out ${c.checkOut}` : ''}`);
    }
  } else {
  ...
  lines.push(`⏳ Not checked in: ${notCheckedIn.length ? notCheckedIn.join(', ') : 'none 🎉'}`);
