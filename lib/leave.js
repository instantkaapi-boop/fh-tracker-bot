...
// All leave entries for a single date (no person filter) — lets check-in
// nagging skip people who are on leave today.
export async function leavesForDate(date = todayIST()) {
  const res = await notion.dataSources.query({
    data_source_id: DATA_SOURCE_ID,
